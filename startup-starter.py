    ...
    def do_GET(self):
        try:
            ...
            if self.path.endswith("/startups"):
                ...
                for startup in startups:
                    output +="<br>"
                    output += "<a href='/startups/%s/detail'>"%startup.id + startup.name + "</a> "
                    output +="<br>"
            ...
            if self.path.endswith("/detail"):
                ...
                if startup:
                    ...
                    output = ""
                    output += "<html><body>"
                    output += "<a> Startup name: </a>" + startup.name
                    output +="<br><br>"
                    output += "<a> Founders name: </a>"
                    output +="<br>"
                    for founder in founders:                        
                        output += founder.name
                        output +="<br>"                    
                    output += "<a href=\"/startups/%s/newfounder\">Add a Founder!</a>"%startup.id
                    output +="<br>"
                    output += "</body></html>"
                    self.wfile.write(output)
            ...
            if self.path.endswith("/newfounder"):
                startup_id = self.path.split("/")[2]
                startup = session.query(Startup).filter_by(id=startup_id).one()
                ...
                if startup:
                    ...
                    output = ""
                    output += "<html><body>"
                    output += "<form method ='POST' enctype='multipart/form-data' action ='/startups/%s/newfounder'>"%startup.id
                    output += "<input type='text' name='name' placeholder='name'/>"
                    output +="<br>"
                    output += "<input type='text' name='bio' placeholder='bio'/>"
                    output +="<br>"
                    output += "<button type='submit'> Add a Founder! </button>"
        ...
        try:
            ...
            if self.path.endswith('/newfounder'):
                ...
                if ctype == 'multipart/form-data':
                    ...
                    self.send_response(301)
                    self.send_header('content-type', "text/html")
                    self.send_header('Location', "/startups/%s/detail"%startup_id)
                    self.end_headers()
